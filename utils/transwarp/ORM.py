...
import logging
import operator
from utils.transwarp import DAO
...
def _gen_sql(table_name, mappings):
    ...
    for f in sorted(mappings.values(), lambda x, y: operator.eq(x._order, y._order)):
        if not hasattr(f, 'ddl'):
            raise BaseException('no ddl in field "%s".' % f)
        ddl = f.ddl
        nullable = f.nullable
        if f.primary_key:
            pk = f.name
...
class Model(dict, metaclass=ModelMetaclass):
    ...
    def get(cls, pk):
        ...
        Get by primary key.
        """
        d = DAO.select_one('select * from %s where %s=?' % (cls.__table__, cls.__primary_key__.name), pk)
    ...
    def find_first(cls, where, *args):
        ...
        """
        d = DAO.select_one('select * from %s %s' % (cls.__table__, where), *args)
        return cls(**d) if d else None
    ...
    def find_by(cls, where, *args):
        ...
        """
        L = DAO.select('select * from `%s` %s' % (cls.__table__, where), *args)
        return [cls(**d) for d in L]
    ...
    def count_all(cls):
        ...
        """
        return DAO.select('select count(`%s`) from `%s`' % (cls.__primay_key__.name, cls.__table__))
    ...
    def count_by(cls, where, *args):
        ...
        """
        return DAO.select_int('select count(`%s`) from `%s` %s' % (cls.__primary_key__.name, cls.__table__, where), *args)
    ...
    def update(self):
        ...
        DAO.update('update `%s` set %s where %s=?' % (self.__table__, ','.join(L), pk), *args)
        return self

    def delete(self):
        """
        通过DAO对象的 update接口 执行SQL
            SQL: delete from `user` where `id`=%s, ARGS: (10190,)
        """
        self.pre_delete and self.pre_delete()
        pk = self.__primary_key__.name
        args = (getattr(self, pk), )
        DAO.update('delete from `%s` where `%s`=?' % (self.__table__, pk), *args)
        return self

    def insert(self):
        """
        通过DAO对象的insert接口执行SQL
            SQL: insert into `user` (`passwd`,`last_modified`,`id`,`name`,`email`) values (%s,%s,%s,%s,%s),
            　　　　　 ARGS: ('******', 1441878476.202391, 10190, 'Michael', 'orm@DAO.org')
        """
        ...
        DAO.insert('%s' % self.__table__, **params)
        return self
...
if __name__ == '__main__':
    ...
    u = User(id=10190, name='Michael', email='orm@DAO.org')
    r = u.insert()
    u.email
    u.passwd
